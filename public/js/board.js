  ...
  xhr.onreadystatechange = function (_) {
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        ...
        console.log("Problem when updating order: ".concat(xhr.responseText));
      }
    }
  };
  xhr.send(data);
...
_defineProperty(Ajax, "getTaskData", function (tarefa) {
  console.log('Taking data from the task...');
  var host = window.location.protocol + '//' + window.location.host;
  ...
  xhr.onreadystatechange = function (_) {
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        ...
        console.log("Problem when fetching task data: ".concat(xhr.responseText));
      }
    }
  };
  xhr.send();
